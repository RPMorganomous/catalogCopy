...
def newRestaurant(): # check PASS
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("New Restaurant created!") # STEP 6 - API AND FLASHING
...
def editRestaurant(restaurant_id): # check PASS
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            session.commit()
            flash("Restaurant successfully edited.") # STEP 6 - API AND FLASHING
            return redirect(url_for('showRestaurants'))
    else:
        return render_template(
...
def deleteRestaurant(restaurant_id): # check PASS
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("Restaurant successfully deleted.") # STEP 6 - API AND FLASHING
        return redirect(
